...
if (process.env.SUPABASE_URL) {
  ...
  if (supabaseKey) {
    try {
      supabase = (0, import_supabase_js.createClient)(process.env.SUPABASE_URL, supabaseKey, {
...
function mapDeviceToDb(d) {
  const db = {};
  if (d.id !== void 0) db.id = d.id;
  if (d.name !== void 0) db.name = d.name;
  if (d.brand !== void 0) db.brand = d.brand;
  if (d.type !== void 0) db.type = d.type;
  if (d.ip !== void 0) db.ip = d.ip;
  if (d.mac !== void 0) db.mac = d.mac;
  if (d.connectionStatus !== void 0) db.connection_status = d.connectionStatus;
  if (d.connectionType !== void 0) db.connection_type = d.connectionType;
  if (d.signalStrength !== void 0) db.signal_strength = d.signalStrength;
  if (d.uploadSpeed !== void 0) db.upload_speed = d.uploadSpeed;
  if (d.downloadSpeed !== void 0) db.download_speed = d.downloadSpeed;
  if (d.category !== void 0) db.category = d.category;
  if (d.lastActive !== void 0) db.last_active = d.lastActive;
  if (d.isBlockedByPolicy !== void 0) db.is_blocked_by_policy = d.isBlockedByPolicy;
  if (d.registeredAt !== void 0 && devicesColumns.registered_at) db.registered_at = d.registeredAt;
  if (d.riskRating !== void 0 && devicesColumns.risk_rating) db.risk_rating = d.riskRating;
  if (d.riskDetails !== void 0 && devicesColumns.risk_details) db.risk_details = d.riskDetails;
  return db;
}
function mapScheduleToFront(s) {
  return {
    id: s.id,
...
function mapScheduleToDb(s) {
  const db = {};
  if (s.id !== void 0) db.id = s.id;
  if (s.deviceId !== void 0) db.device_id = s.deviceId;
  if (s.deviceName !== void 0) db.device_name = s.deviceName;
  if (s.name !== void 0) db.name = s.name;
  if (s.days !== void 0) db.days = s.days;
  if (s.startTime !== void 0) db.start_time = s.startTime;
  if (s.endTime !== void 0) db.end_time = s.endTime;
  if (s.isEnabled !== void 0) db.is_enabled = s.isEnabled;
  return db;
}
function mapLogToFront(l) {
  return {
    id: l.id,
...
var logs = [
  ...
  {
    id: "log-003",
    timestamp: new Date(Date.now() - 36e5 * 1.2).toISOString(),
...
var STORAGE_FILE = import_path.default.join(process.cwd(), "local_storage.json");
function saveLocalState() {
  try {
    ...
    import_fs.default.writeFileSync(STORAGE_FILE, JSON.stringify(data, null, 2), "utf-8");
  } catch (err) {
...
loadLocalState();
async function addLog(deviceName, deviceMac, type, details, severity) {
  const newLog = {
...
async function syncAndLoadWithSupabase() {
  ...
  try {
    ...
    if (testErr) {
      if (testErr.code === "PGRST116" || testErr.code === "42P01" || testErr.message?.includes("relation") || testErr.message?.includes("does not exist")) {
